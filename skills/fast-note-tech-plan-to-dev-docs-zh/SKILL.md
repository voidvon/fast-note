---
name: fast-note-tech-plan-to-dev-docs-zh
description: 将 Fast-Note 的“功能拆分与技术方案”文档转换为开发执行文档（迭代计划与任务看板、组件与数据契约清单、测试与发布计划）。当用户提出“从方案到开发排期/任务拆解/组件设计/测试发布落地”类需求，或需要把 `doc/功能拆分与技术方案/` 产物转为可执行研发计划时使用。
---
...
1. 读取输入文档：
   - `doc/功能拆分与技术方案/Fast-Note-功能拆分与技术方案-总览-*.md`
   - `doc/功能拆分与技术方案/Fast-Note-功能拆分与技术方案-<模块名>-*.md`
2. 按模块抽取：功能点、任务拆分、验收标准、风险、未决问题。
3. 输出 3 份开发文档到 `doc/开发计划/`：
   - `迭代计划与任务看板.md`
   - `组件与数据契约清单.md`
   - `测试与发布计划.md`
...
- 强制覆盖这些层：`views/components`、`hooks`、`stores`、`database`、`pocketbase`、`router`。
...
- UI 不直接调 PocketBase，核心数据改动必须经过 store/hook 编排。
...
- 测试计划中必须包含可执行命令与通过标准（例如 `npm test`、`npm run test:e2e`）。

## 推荐检索命令

```bash
find doc/功能拆分与技术方案 -maxdepth 1 -type f | sort
rg "任务ID|验收标准|风险|未决问题|里程碑" doc/功能拆分与技术方案
rg "views/components|hooks|stores|database|pocketbase|router" doc/功能拆分与技术方案
```
